...
const PinnedTilesSection: FC<Props> = ({pinnedTile, unpin, webrtc, showSimulcast}: Props) => {
    ...
    return (<div className="active-screensharing-grid h-full grid-cols-1">
    <MediaPlayerTile
      ...
      layers={<>
        { (hasInitials && showDisabledIcon(pinnedTile.video)) ? <InitialsImage initials={pinnedTile.initials} /> : null}
        <PeerInfoLayer 
          topRight={<PinIndicator/>}
          topLeft={(hasInitials && showDisabledIcon(pinnedTile.audio) )?
